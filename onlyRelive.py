...
def keyDown(windowProcess,name):
    name = int(name)
    win32api.SendMessage(windowProcess, win32con.WM_KEYDOWN,name)#模拟鼠标按下
    win32api.SendMessage(windowProcess, win32con.WM_KEYUP, name)#模拟鼠标弹起
# 鼠标点击事件函数
...
# login
def login():
    # f = open(r'./userKey.txt','w', encoding='utf-8')
    path = './userKey.txt'
    ...
    nowTime = f"""{nowTime.tm_year}{0 if nowTime.tm_mon < 10 else ''}{nowTime.tm_mon}{nowTime.tm_mday}"""
    try:
        respones = requests.get("http://ge1.xyz/DataBase/userTime.json",timeout=10)
    except:
        print('获取数据失败...')
        time.sleep(1)
        os._exit()
    else:
        userJson = json.loads(respones.text)
        statelocal = False
        if localKey in userJson.keys():
            if int(userJson[localKey]) > int(nowTime):
                statelocal = True
        if not statelocal:
            print('密钥已过期...')
            key = input('请输入密钥:')
            ...
            if key in userJson.keys():
                if int(userJson[key]) > int(nowTime):
                    state = True
                    fw = open(path,'w+', encoding='utf-8')
                    fw.write(key)
                    f.close()
